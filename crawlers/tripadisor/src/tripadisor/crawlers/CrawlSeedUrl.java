...
public class CrawlSeedUrl {
	...
	public Map<String, String> crawlThingsToDo(String segment) {
		...
		while (nextPage && members.size() < 250) {
			try {
				...
				Elements pageList = doc.select("div#pager_top").select("a");
				String val = pageList.get(pageList.size() - 1).attr("href");
	...
	public JSONObject processWebPages(File url, String name) {
		...
		try {
			...
			Elements reviews = doc.select("p.partial_entry");
			int count = 0; //
			data.clear();
			for (Element element2 : reviews) {
				data.put("review", element2.text());
				arrayElementOneArray.add(new CreateJsonRead()
						.createJsonObect(data));
				break;
			}
